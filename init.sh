...
[[ -n "$F2B_IGNOREIP" ]] && IGNOREIP="$IGNOREIP $F2B_IGNOREIP"

# 直接写入干净的 jail.local，不从 jail.conf 复制（避免 [sshd] 段重复）
# 幂等：每次重新生成
cat > "$JAIL_LOCAL" <<EOF
# Generated by init_vps.sh
...
# ──────────────────────────────────────────────
# Step 12：配置 Swap 虚拟内存
# ──────────────────────────────────────────────
step "12 / 12  配置 Swap 虚拟内存"

CURRENT_SWAP=$(swapon --show=SIZE,NAME --noheadings 2>/dev/null || true)

if [[ -n "$CURRENT_SWAP" ]]; then
  CURRENT_SWAP_SIZE=$(free -h | awk '/^Swap:/ {print $2}')
  info "当前系统已有 Swap：$CURRENT_SWAP_SIZE"
  read -rp "$(echo -e "${BOLD}是否修改 Swap 大小？[y/N]：${RESET}")" CHANGE_SWAP
  CHANGE_SWAP="${CHANGE_SWAP,,}"

  if [[ "$CHANGE_SWAP" == "y" ]]; then
    while true; do
      read -rp "$(echo -e "${BOLD}请输入新的 Swap 大小（例如 1G、2G、512M）：${RESET}")" SWAP_SIZE
      [[ "$SWAP_SIZE" =~ ^[0-9]+(M|G|m|g)$ ]] && break
      warn "格式不正确，请输入如 1G、2G、512M 的格式"
    done
    ...
    info "正在创建 ${SWAP_SIZE} Swap..."
    fallocate -l "$SWAP_SIZE" /swapfile
    chmod 600 /swapfile
    mkswap /swapfile
    swapon /swapfile
    echo '/swapfile none swap sw 0 0' >> /etc/fstab
    success "Swap 已调整为 $SWAP_SIZE"
    SWAP_FINAL="$SWAP_SIZE"
  else
    info "保持现有 Swap 不变"
    SWAP_FINAL="$CURRENT_SWAP_SIZE（未修改）"
  fi

else
  info "当前系统没有 Swap"
  read -rp "$(echo -e "${BOLD}是否创建 Swap？[y/N]：${RESET}")" CREATE_SWAP
  CREATE_SWAP="${CREATE_SWAP,,}"

  if [[ "$CREATE_SWAP" == "y" ]]; then
    while true; do
      read -rp "$(echo -e "${BOLD}请输入 Swap 大小（例如 1G、2G、512M）：${RESET}")" SWAP_SIZE
      [[ "$SWAP_SIZE" =~ ^[0-9]+(M|G|m|g)$ ]] && break
      warn "格式不正确，请输入如 1G、2G、512M 的格式"
    done
